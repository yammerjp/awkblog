...
function oauth_callback__get() {
  ...
  if (code == "" || error != "") {
    http::render_html(500, "failed")
  }
  # TODO verify state
  ret = lib::command_exec("curl -X POST -H 'Accept: application/json' 'https://github.com/login/oauth/access_token?client_id=" AWKBLOG_OAUTH_CLIENT_KEY "&client_secret=" AWKBLOG_OAUTH_CLIENT_SECRET "&code=" code "'")
  access_token = lib::json_extract(ret, "access_token")
  if (access_token == "") {
    http::render_html(500, "access token is not found")
  }
